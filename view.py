...
city_need_energy_begin = generate_field("от", X1, mass[20], X2, mass[20])
city_need_energy_end = generate_field("до", X1, mass[22], X2, mass[21])
...
cost_gasifiers = generate_field("Стоимость газификаторов ", X1, mass[7], X2, mass[7])
...
factory_distance=generate_field("Расстояние от завода до города", X1, mass[8], X2, mass[8])
gas_material = Label(root, text="Материал газопровода", background="white") 
gas_material.place(x=X1,y=mass[9])
...
btplot1.place(x=X1, y=mass[11], width=150/1.5, height=60/1.5)
...
btplot2.place(x=X1, y=mass[14], width=150/1.5, height=60/1.5)
...
btplot2.place(x=X1, y=mass[17], width=150/1.5, height=60/1.5)
