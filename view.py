...
cost_natur_liquided_gas = generate_field("Стоимость сжиженного природного газа(СПГ), руб/тонна", X1, mass[1], X2,
                                         mass[1])
lbl = Label(root, text="Годовой объем потребления, МВт*ч/год", background="white")
...
volume_cistern = generate_field("Объем криогенной цистерны СПГ, м^3", X1, mass[6], X2, mass[6])
cost_tank = generate_field("Стоимость хранилища СПГ, руб", X1, mass[8], X2, mass[8])
volume_tank = generate_field("Объем хранилища СПГ, м^3", X1, mass[9], X2, mass[9])
