...
1cost_gas = generate_field("Стоимость газа", X1, mass[0], X2, mass[0])
1cost_natur_liquided_gas = generate_field("Стоимость СПГ", X1, mass[1], X2, mass[1])
#city_need_energy = generate_field("Потребность города в КВТ", X1, mass[2], X2, mass[2])
1cost_cistern = generate_field("Стоимость автомобильной цистерны", X1, mass[3], X2, mass[3])
#volume_cistern = generate_field("Объем автомобильной цистерны", X1, mass[4], X2, mass[4])
1number_cistern = generate_field("Колличество цистерн ", X1, mass[4], X2, mass[4])
1cost_tank = generate_field("Стоимость хранилища СПГ", X1, mass[5], X2, mass[5])
#volume_tank = generate_field("Объем хранилища СПГ", X1, mass[6], X2, mass[6])
1number_tank = generate_field("Колличество хранилищ ", X1, mass[7], X2, mass[7])
1cost_gasifiers = generate_field("Стоимость газификаторов ", X1, mass[8], X2, mass[8])
...
1factory_distance=generate_field("Расстояние от завода до города", X1, mass[9], X2, mass[9])
